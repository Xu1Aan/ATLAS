...
def _get_dxf_extent(dxf_path: Path) -> tuple[float, float, float, float] | None:
    """Read the DXF extent using ogrinfo."""
    ok, out = _run(["ogrinfo", "-ro", "-so", str(dxf_path), "entities"])
    if not ok:
        return None
    match = re.search(
...
def convert_dwg_to_gpkg(dwg_path: Path, output_dir: Path, progress_callback=None) -> tuple[bool, Path | None, str]:
    ...
    dxf_path = output_dir / "source.dxf"
    ...
    if settings.enable_gauss_kruger_transform:
        ...
        dxf_extent = _get_dxf_extent(dxf_path)
        if dxf_extent:
            min_x, _, max_x, _ = dxf_extent
            max_abs_x = max(abs(min_x), abs(max_x))
            if max_abs_x < 1000000:
                false_easting = 500000
            print(f"DXF extent detected: min_x={min_x:.3f}, max_x={max_x:.3f}, false_easting={false_easting}")
...
def get_gpkg_bbox(gpkg_path: Path) -> tuple[bool, list[float] | None]:
    ...
    try:
        cmd = ["ogrinfo", "-ro", "-so", str(gpkg_path), "entities"]
        ok, out = _run(cmd)
        if not ok:
            return False, None
        match = re.search(
